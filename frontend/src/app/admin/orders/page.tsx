...
export default function ManageOrders() {
  ...
  async function handleDelete(orderId: string) {
    ...
    try {
      const res = await fetch(`http://localhost:3001/orders/admin/orders/${orderId}`, {
        method: "DELETE",
        headers: { Authorization: `Bearer ${user?.token}` },
      });
      ...
      <table className="w-full mt-6 bg-gray-800 text-white rounded-lg">
        <thead>
          <tr className="bg-blue-500">
            ...
            <th className="p-3">Tracking</th> {/* ✅ Display Tracking Number */}
        ...
        <tbody>
          {orders.length === 0 ? (
            <tr>
              <td colSpan={5} className="text-center p-3">No orders found</td> {/* ✅ Fix colSpan */}
            </tr>
          ) : (
            orders.map((order) => (
