...
export default function SuccessPage() {
  ...
  useEffect(() => {
    ...
    if (pickup) {
      // Local pickup flow: just read orderId from query or local storage and show toast
      const oid = searchParams.get("order_id") || localOrder?.orderId || null;
      setOrderId(oid);
      toast.success("🎉 Pickup order reserved for 48 hours");
      return;
    }
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-black text-white">
      <div className="text-center p-6 bg-gray-800 rounded-lg shadow-lg w-[500px]">
        ...
        <p className="mt-4">{pickup ? "We’ll hold your items for 48 hours." : "Your order has been successfully placed."}</p>
        ...
        {pickup && localOrder?.expiresAt && (
          <p className="mt-1 text-fg/70">
            Reserved until: {new Date(localOrder.expiresAt).toLocaleString()}
          </p>
        )}
