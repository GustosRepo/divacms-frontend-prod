    ...
    <div className="pt-32 md:pt-36 pb-10 space-y-10">
      ...
      <section className={`relative overflow-hidden rounded-3xl p-10 bg-gradient-to-br ${theme.gradient}`}>
        <div className="relative z-10 max-w-3xl">
          <h1 className="text-4xl md:text-5xl font-extrabold tracking-tight outline-heading text-legible">
            {displayName.split(' ').slice(0, 2).join(' ')}{' '}
            <span className={`${theme.accent} outline-heading text-legible`}>{displayName.split(' ').slice(2).join(' ') || ''}</span>
          </h1>
          <p className="mt-4 text-lg md:text-xl font-medium text-legible">{tagline}</p>
          <div className="mt-6 flex flex-wrap gap-3">
            {tags.map(tag => (
              <span key={tag} className={`px-3 py-1 rounded-full text-xs font-semibold border ${theme.chipBg} text-legible`}>{tag}</span>
            ))}
          </div>
      ...
      {categories?.length > 0 && (
        <section>
          <h2 className="text-2xl font-bold mb-4 outline-heading text-legible">Featured Categories</h2>
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {categories.map(cat => (
              <Link
                key={cat.slug}
                href={`/shop?brand_segment=${brand}&category=${cat.slug}`}
                className={`group rounded-xl p-6 ${theme.cardBg} transition hover:-translate-y-1 hover:shadow-xl dark:shadow-black/40`}
              >
                <h3 className="font-semibold text-lg group-hover:opacity-80 text-legible">{cat.title}</h3>
                <p className="mt-1 text-sm text-legible">{cat.desc}</p>
      ...
      {/* Value Props */}
      <section>
        <h2 className="text-2xl font-bold mb-4 outline-heading text-legible">Why Shop With Us?</h2>
        <div className="grid md:grid-cols-3 gap-6">
          {[{ title: 'Curated Drops', text: 'Hand-picked designers & emerging studios.' }, { title: 'Authenticity First', text: 'Guaranteed genuine limited pieces.' }, { title: 'Fast & Secure Shipping', text: 'Protected packaging for mint condition.' }].map(f => (
            <div key={f.title} className={`p-6 rounded-xl ${theme.cardBg} dark:shadow-black/40`}>
              <h3 className="font-semibold text-legible">{f.title}</h3>
              <p className="mt-2 text-sm leading-relaxed text-legible">{f.text}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <section>
        <h2 className="text-2xl font-bold mb-4 outline-heading text-legible">Next Steps</h2>
        <ol className="space-y-2 text-sm list-decimal list-inside text-legible">
          <li>Assign products a brand_segment = "{brand}" (add during creation or migration).</li>
          <li>Filter /shop page by brand_segment param for segmentation.</li>
          <li>Add dynamic category pages later for deeper taxonomy.</li>
        </ol>
      </section>
    </div>
  );
